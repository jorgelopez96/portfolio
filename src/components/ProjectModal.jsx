...
export default function ProjectModal({ project, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div
        className="relative z-10 w-full max-w-4xl bg-[#0f0f1a] border border-purple-700/40 rounded-2xl overflow-hidden shadow-2xl shadow-purple-900/30 flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-purple-900/30">
          <div>
            <p className="font-mono text-xs text-purple-500 tracking-widest uppercase mb-0.5">{project.institution}</p>
            <h3 className="font-display font-bold text-white text-lg">{info.name}</h3>
          </div>
          <div className="flex items-center gap-3">
            <a href={project.github} target="_blank" rel="noreferrer"
              className="font-mono text-xs text-gray-400 border border-white/10 px-3 py-1.5 rounded-xl hover:border-purple-500/50 hover:text-purple-400 transition-all">
              GitHub
            </a>
        ...
        <div className="overflow-hidden bg-black">
          <img src={project.image} alt={info.name} className="w-full object-cover object-top max-h-[65vh]" />
        </div>
